...
class Staff(commands.Cog):
    ...
    async def _staff_warn(self, ctx, member: disnake.Member, *, reason):
        ...
        embed = disnake.Embed(
            color=0x56C9F0,
            title="Warning",
            description=f":warning: **`[ WARNING ]`** {member.mention} `[ {member.id} ]` [**WARNING REASON PASTEBIN**]({document.url})",
            timestamp=datetime.utcnow(),
        )
        embed.add_field(
            name="Warning Reason", value=f"```fix\n{reason}\n```", inline="false"
        )
        embed.set_author(
        ...
        await member.send(
            f"{member.mention}, You have been given a __staff warning__ in Glacier Advertising!\n The details are listed below!"
        )
    ...
    async def _staff_strike(self, ctx, member: disnake.Member, *, reason):
        ...
        embed = disnake.Embed(
            color=0x56C9F0,
            title="Strike",
            description=f":triangular_flag_on_post: **`[ STRIKE ]`** {member.mention} `[ {member.id} ]` [**STRIKE REASON PASTEBIN**]({document.url})",
            timestamp=datetime.utcnow(),
        )
        embed.add_field(
            name="Strike Reason", value=f"```fix\n{reason}\n```", inline="false"
        )
        embed.set_author(
            name="Glacier Moderation",
            icon_url="https://cdn.discordapp.com/avatars/851592943075721268/58549a661b24ec550d5091a11599a030.png?size=1024",
        ...
        await channel2.send(embed=embed)
        await member.send(
            f"{member.mention}, You have been given a __staff strike__ in Glacier Advertising!\n The details are listed below!"
        )
        await member.send(member.mention, embed=embed)
    ...
    async def _staff_log_demote(
        ...
        *,
        ...
        embed = disnake.Embed(
            color=0xFFC16B,
            title="Demotion",
            description=f":small_red_triangle_down: **`[ DEMOTION ]`** {member.mention} `[ {member.id} ]` [**DEMOTION REASON PASTEBIN**]({document.url})",
            timestamp=datetime.utcnow(),
        )
        embed.add_field(name="Old Role", value=f"` {oldrole}  `", inline="true")
        embed.add_field(name="New Role", value=f"` {newrole}  `", inline="true")
        embed.add_field(
            name="Demotion Reason", value=f"```fix\n{reason}\n```", inline="false"
        )
        embed.set_author(
            name="Glacier Moderation",
            icon_url="https://cdn.discordapp.com/avatars/851592943075721268/58549a661b24ec550d5091a11599a030.png?size=1024",
        ...
        embed2 = disnake.Embed(
            color=0xF75262,
            title="Demoted",
            description=":small_red_triangle_down: This staff member has been __DEMOTED__.",
            timestamp=datetime.utcnow(),
        )
        embed2.add_field(
            name="<:GA_Member:920077003842019338> Member",
            value=f"{member.mention}",
            inline="true",
        )
        embed2.add_field(
            name="<:GA_ID:920101219215745025> Member ID",
            value=f"**`[{member.id}]`**",
            inline="true",
        )
        embed2.add_field(
            name="<:GA_Ban:920078316067762266> Approval",
        ...
        embed2.set_thumbnail(
            url="https://creazilla-store.fra1.digitaloceanspaces.com/emojis/48164/tomato-emoji-clipart-sm.png"
        )
        embed2.set_footer(
            text=f"Requested by {ctx.author}", icon_url=ctx.author.avatar.url
        )
        ...
        await channel2.send(embed=embed)
        await member.send(
            f"{member.mention}, You have been __demoted__ in Glacier Advertising!\n The details are listed below!"
        )
        await member.send(member.mention, embed=embed)
    ...
    async def _staff_log_fire(self, ctx, member: disnake.Member, *, reason):
        ...
        embed2 = disnake.Embed(
            color=0xF75262,
            title="Fired",
            description=":tomato: This staff member has been __FIRED__.",
            timestamp=datetime.utcnow(),
        )
        embed2.add_field(
            name="<:GA_Member:920077003842019338> Member",
        ...
        embed2.add_field(
            name="Firing Reason", value=f"```fix\n{reason}\n```", inline="false"
        )
        embed2.set_author(
            name="Glacier Moderation",
            icon_url="https://cdn.discordapp.com/avatars/851592943075721268/58549a661b24ec550d5091a11599a030.png?size=1024",
        ...
        await member.send(
            f"{member.mention}, You have been __fired__ in Glacier Advertising!\n The details are listed below!"
        )
